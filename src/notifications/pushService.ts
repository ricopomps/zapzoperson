...
export async function registerPushNotifications() {
  ...
  if (existingSubscription) throw Error("Existing push subscription found");
...
export async function sendPushSubscriptionToServer(
  subsctiption: PushSubscription
) {
  const response = await fetch("/api/register-push", {
    method: "POST",
    body: JSON.stringify(subsctiption),
  });
  if (!response.ok) throw Error("Failed to send push subscription to user");
}
export async function deletePushSubscriptionFromServer(
  ...
  if (!response.ok) throw Error("Failed to delete push subscription to user");
}
